...
trainingFaces=readImage('D:\\deepayan\\study\\study\\sem 2\\ECE 763\\proj1\\train_face')
trainingNonFaces=readImage('D:\\deepayan\\study\\study\\sem 2\\ECE 763\\proj1\\train_non_face')
testingface=readImage('D:\\deepayan\\study\\study\\sem 2\\ECE 763\\proj1\\test_face')
testingnonface=readImage('D:\\deepayan\\study\\study\\sem 2\\ECE 763\\proj1\\test_non_face')
...
def fitting_MixtureT(x,mu,Sigma,df,d):
    Num = gamma(1. * (d+df)/2)
...
def Mixture_of_T_distribution(X, dof=3.5, iter=100, eps=1e-1):
...
parameters=Mixture_of_T_distribution(trainingFaces, dof=4.0, iter=50, eps=1e-1)
parameters1=Mixture_of_T_distribution(trainingNonFaces,dof=4.0,iter=50,eps=1e-1)
...
def evaluate_Model(testingface,testingnonface):
    ...
    for i in range(0,testsize):
        a=fitting_MixtureT(testingface[i],parameters[1],parameters[0],dof,60)
        b=fitting_MixtureT(testingface[i],parameters1[1],parameters1[0],dof,60)
        y=np.true_divide(a,(a+b))
        if (y>0.5):
            predictions1.append(y)
            TP=TP+1
    new_TP1=[0]*test_size
    ...
    for i in range(0,testsize):
        a=fitting_MixtureT(testingnonface[i],parameters[1],parameters[0],dof,60)
        b=fitting_MixtureT(testingnonface[i],parameters1[1],parameters1[0],dof,60)
        y=np.true_divide(a,(a+b))
        if (y>0.5):
...
def plot_ROC(f_nf, f_f,test_size):
    ...
    plt.title('ROC Model_5 Mixutre of T Distribution')
